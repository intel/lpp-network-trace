...
function calculateBandwidth(bwArray) {
  ...
  log('debug', 'sorted: ' + sorted, 'calculateBandwidth'); // remove bottom ~22% of results
  ...
  if (filtered.length > 2) {
    ...
    log('debug', 'average bandwidth: ' + ret + ' kbps', 'calculateBandwidth');
  } else {
    // not enough data -> calculate total average value
    ret = Math.floor(totalReceivedBytes * 8 / 1024 / totalDurationSec);
    log('debug', 'total average bandwidth: ' + ret + ' kbps', 'calculateBandwidth');
  }
...
function getDlBw(limitKbytes, maxTimeSec, url, onprogress) {
  ...
  precondition(Number.isInteger(+limitKbytes), 'limitKbytes is not an integer');
  precondition(+limitKbytes > 0, 'limitKbytes <= 0');
  precondition(maxTimeSec !== null, 'maxTimeSec=null');
  precondition(Number.isInteger(+maxTimeSec), 'maxTimeSec is not an ' + 'integer');
  ...
  precondition(onprogress === null || onprogress === undefined || isFunction(onprogress), 'onprogress is not a function');
  ...
  return new Promise(function (resolve, reject) {
    ...
    request.onprogress = function (result) {
      ...
      if (DEBUG === true) {
        log('debug', 'received bytes: ' + receivedBytes + ', duration: ' + durationSec + ' sec' + ', total bytes: ' + totalReceivedBytes + ', bw: ' + kbps + ' kbps', 'XMLHttpRequest.onprogress');
      }

      totalDurationSec = (currentTimeMillis - startTimeMillis) / 1000;

      if (onprogress !== undefined && onprogress !== null) {
        const dlProgress = Math.floor(totalReceivedBytes / (limitKbytes * 1024) * 100);
        const timeProgress = Math.floor(totalDurationSec / maxTimeSec * 100);
      ...
      if (totalReceivedBytes >= limitKbytes * 1024 || totalDurationSec > maxTimeSec) {
        ...
        log('info', 'final bandwidth: ' + finalBandwidthKbps + ' kbps', 'XMLHttpRequest.onprogress');
      ...
      if (request.readyState === XMLHttpRequest.DONE) {
        ...
        if (status >= HTTP_OK && status < HTTP_BAD_REQUEST) {
          ...
          if (bandwidthKbpsArray.length >= 1) {
            finalBandwidthKbps = calculateBandwidth(bandwidthKbpsArray);
            log('info', 'final bandwidth: ' + finalBandwidthKbps + ' kbps', 'XMLHttpRequest.onreadystatechange');
    ...
    timer = window.setTimeout(function () {
      ...
      if (finalBandwidthKbps >= 0) {
        log('info', 'final bandwidth: ' + finalBandwidthKbps + ' kbps', 'timer');
