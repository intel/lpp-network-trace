...
Date.prototype.toIsoString = function () {
  ...
  return this.getFullYear() + '-' + pad(this.getMonth() + 1) + '-' + pad(this.getDate()) + 'T' + pad(this.getHours()) + ':' + pad(this.getMinutes()) + ':' + pad(this.getSeconds()) + '.' + pad(this.getMilliseconds(), 3) + dif + pad(tzo / 60) + ':' + pad(tzo % 60);
};
...
class Trace {
  ...
  constructor(version, description, clientModel, clientName, note, settings) {
    ...
    if (typeof clientModel !== 'undefined' && clientModel !== null) {
      assert(typeof clientModel === 'string', 'clientModel is not string');
      this.clientModel = clientModel;
    }

    if (typeof clientName !== 'undefined' && clientName !== null) {
      assert(typeof clientName === 'string', 'clientName is not string');
    ...
    if (settings !== null && typeof settings === 'object') {
      if (DEBUG === true) {
        if ("dlBwTestDuration" in settings) {
          assert(Number.isInteger(+settings.dlBwTestDuration), 'dlBwTestDuration is not an integer');
          assert(+settings.dlBwTestDuration > 0, 'dlBwTestDuration is not positive');
        }
      }
      ...
      if (DEBUG === true) {
        if ("dlBwTestDownloadLimit" in settings) {
          assert(Number.isInteger(+settings.dlBwTestDownloadLimit), 'dlBwTestDownloadLimit is not an integer');
          assert(+settings.dlBwTestDownloadLimit > 0, 'dlBwTestDownloadLimit is not positive');
        }
      }
      ...
      if (DEBUG === true) {
        if ("dlBwTestUrl" in settings) {
          assert(typeof settings.dlBwTestUrl === 'string', 'dlBwTestUrl is not a string');
        }
      }
      ...
      if (DEBUG === true) {
        if ("ulBwTestDuration" in settings) {
          assert(Number.isInteger(+settings.ulBwTestDuration), 'ulBwTestDuration is not an integer');
          assert(+settings.ulBwTestDuration > 0, 'ulBwTestDuration is not positive');
        }
      }
      ...
      if (DEBUG === true) {
        if ("ulBwTestUploadLimit" in settings) {
          assert(Number.isInteger(+settings.ulBwTestUploadLimit), 'ulBwTestUploadLimit is not an integer');
          assert(+settings.ulBwTestUploadLimit > 0, 'ulBwTestUploadLimit is not positive');
        }
      }
...
class Position {
  ...
  constructor(latitude, longitude, accuracy) {
    ...
    precondition(accuracy !== null && !isNaN(accuracy), 'accuracy is not a number');
...
class PositionHandler {
  ...
  getPosition() {
    ...
    let watchPosition = function () {
      var showPosition = function (position) {
        const pos = new Position(position.coords.latitude, position.coords.longitude, position.coords.accuracy);
        log('debug', 'got new position: ' + pos.getLatitude() + ' ' + pos.getLongitude() + ' (acc: ' + pos.getAccuracy() + ')', 'PositionHandler::showPosition');
        this.position = pos;
      };
      ...
      var errorPosition = function (error) {
        ...
        this.position = null;
        log('err', 'failed to get position: error: ' + e, 'PositionHandler::errorPosition');
      };
      ...
      if (navigator.geolocation) {
        this.watchPositionId = navigator.geolocation.watchPosition(showPosition.bind(this), errorPosition.bind(this), options);
        log('debug', 'watchPositionId: ' + this.watchPositionId, 'PositionHandler::watchPosition');
      } else {
        this.error = 'geolocation not supported';
        this.position = null;
        log('warn', 'geo not supported', 'PositionHandler::watchPosition');
      }
    }.bind(this);
    ...
    let queryGeoLocationPermission = function () {
      return new Promise(function (resolve, reject) {
        if (navigator.permissions) {
          log('debug', 'navigator.permissions exists', 'PositionHandler::queryGeoLocationPermission');
          navigator.permissions.query({
            name: 'geolocation'
          }).then(function (permissionStatus) {
            log('debug', 'geolocation permission state is: ' + permissionStatus.state, 'PositionHandler::queryGeoLocationPermission');
    ...
    let getCurrentPosition = function () {
      return new Promise(function (resolve) {
        ...
        var errorPosition = function (error) {
          ...
          this.position = null;
          log('err', 'failed to get geolocation: error: ' + e, 'PositionHandler::errorPosition');
          resolve(null);
        };
        ...
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(showPosition.bind(this), errorPosition.bind(this), options);
        } else {
          this.error = 'geolocation not supported';
          this.position = null;
          log('warn', 'geo not supported', 'PositionHandler::getCurrentPosition');
          resolve(null);
        }
      }.bind(this));
    }.bind(this);
    ...
    return new Promise(async function (resolve) {
      this.timer = window.setTimeout(function () {
        log('err', 'position not available (timed out)', 'PositionHandler::getCurrentPosition');
        resolve(null);
      ...
      try {
        ...
        if (permState === 'granted') {
          if (this.watchPositionId !== null && this.position !== null) {
            clearTimeout(this.timer); // return last received position

            log('info', 'position: ' + this.position.getLatitude() + ' ' + this.position.getLongitude() + ' (acc: ' + this.position.getAccuracy() + ')', 'PositionHandler::getPosition');
            resolve(this.position);
            return;
          }

          if (this.watchPositionId === null) {
            log('debug', 'start watching position changes', 'PositionHandler::getPosition');
            watchPosition();
          }
          ...
          if (pos !== null) {
            log('info', 'position: ' + pos.getLatitude() + ' ' + pos.getLongitude() + ' (acc: ' + pos.getAccuracy() + ')', 'PositionHandler::getPosition');
          } else {
            log('err', 'position not available', 'PositionHandler::getCurrentPosition');
          }
          ...
          if (this.watchPositionId !== null) {
            log('debug', 'stop watching position changes', 'PositionHandler::getPosition');
  ...
  stopPosition() {
    ...
    if (this.watchPositionId !== null) {
      log('debug', 'stop watching position changes', 'PositionHandler::stopPosition');
      navigator.geolocation.clearWatch(this.watchPositionId);
      this.watchPositionId = null;
    }
...
class TraceEntry {
  ...
  constructor(entryNo, dateTime, opt) {
    ...
    if (options.dlBw !== undefined) {
      assert(Number.isInteger(+options.dlBw), 'dlBw is not an integer');
      this.dlBw = options.dlBw;
    }

    if (options.dlLatency !== undefined) {
      assert(Number.isInteger(+options.dlLatency), 'dlLatency is not an integer');
      this.dlLatency = options.dlLatency;
    }

    if (options.ulBw !== undefined) {
      assert(Number.isInteger(+options.ulBw), 'ulBw is not an integer');
      this.ulBw = options.ulBw;
    }

    if (options.ulLatency !== undefined) {
      assert(Number.isInteger(+options.ulLatency), 'ulLatency is not an integer');
    ...
    if (options.note !== undefined) {
      ...
      assert(ucs2length(options.note) <= Defaults.noteMaxLength, 'note is longer than ' + Defaults.noteMaxLength);
      this.note = options.note;
    }

    if (options.network !== undefined) {
      assert(typeof options.network === 'string', 'network is not a string');
      assert(ucs2length(options.network) <= Defaults.networkMaxLength, 'network is longer than ' + Defaults.networkMaxLength);
...
class Tracer {
   ...
   * (e.g. "dlBwTestDuration", "dlBwTestDownloadLimit", "dlBwTestUrl")
   */
  constructor(schemaVersion, description, clientModel, clientName, note, settings) {
    this.entries = 0;
    this.dump = new Trace(schemaVersion, description, clientModel, clientName, note, settings);
  }
    ...
    if (navigator.connection) {
      ...
      if (typeof type !== 'undefined') {
        if (typeof eff !== 'undefined') {
          options.network = type + ' (Effective connection type: ' + eff + ')';
        } else {
...
class LppNetworkTracer {
  ...
  constructor(schemaVersion, description, options) {
    ...
    precondition(typeof description === 'string', 'description is not a string');
    precondition(ucs2length(description) <= Defaults.descriptionMaxLength, 'description is longer than ' + Defaults.descriptionMaxLength);
    ...
    if (options.traceDlBw !== undefined) {
      precondition(typeof options.traceDlBw === 'boolean', 'traceDlBw is not a boolean');
      _traceDlBw = options.traceDlBw;
    } else {
      _traceDlBw = false;
    }

    this.getTraceDlBw = function () {
      return _traceDlBw;
    };
    ...
    if (options.tracePosition !== undefined) {
      precondition(typeof options.tracePosition === 'boolean', 'tracePosition is not a boolean');
      _tracePosition = options.tracePosition;
    } else {
      _tracePosition = false;
    }

    this.getTracePosition = function () {
      return _tracePosition;
    };

    precondition(this.getTracePosition() === true || this.getTraceDlBw() === true, 'tracing not enabled');

    var _dlBwTestDuration;

    if (options.dlBwTestDuration !== undefined) {
      precondition(Number.isInteger(+options.dlBwTestDuration), 'dlBwTestDuration is not an integer');
      precondition(+options.dlBwTestDuration > 0, 'dlBwTestDuration <= 0');
    ...
    if (options.dlBwTestInterval !== undefined) {
      precondition(Number.isInteger(+options.dlBwTestInterval), 'dlBwTestInterval is not an integer');
      precondition(+options.dlBwTestInterval > 0, 'dlBwTestInterval is not positive');
      precondition(this.getDlBwTestDuration() <= +options.dlBwTestInterval, 'dlBwTestDuration > dlBwTestInterval');
      _dlBwTestInterval = options.dlBwTestInterval;
    } else {
      _dlBwTestInterval = Defaults.dlBwTestInterval;
    }

    this.getDlBwTestInterval = function () {
      return _dlBwTestInterval;
    };

    var _clientModel;

    if (options.clientModel !== undefined) {
      precondition(typeof options.clientModel === 'string', 'clientModel is not a string');
      precondition(ucs2length(options.clientModel) <= Defaults.clientModelMaxLength, 'clientModel is longer than ' + Defaults.clientModelMaxLength);
      _clientModel = options.clientModel;
    } else {
      _clientModel = null;
    }

    this.getClientModel = function () {
      return _clientModel;
    };
    ...
    if (options.clientName !== undefined) {
      precondition(typeof options.clientName === 'string', 'clientName is not a string');
      precondition(ucs2length(options.clientName) <= Defaults.clientNameMaxLength, 'clientName is longer than ' + Defaults.clientNameMaxLength);
    ...
    if (options.note !== undefined) {
      precondition(typeof options.note === 'string', 'note is not a string');
      precondition(ucs2length(options.note) <= Defaults.noteMaxLength, 'note is longer than ' + Defaults.noteMaxLength);
      _note = options.note;
    } else {
      _note = null;
    }
    ...
    if (options.dlLimitKbytes !== undefined) {
      precondition(Number.isInteger(+options.dlLimitKbytes), 'dlLimitKbytes is not an integer');
    ...
    this.setPosition = function (val) {
      precondition(val === null || val instanceof Position, 'pos is not an instance of ' + 'Position class');
      _position = val;
    };

    var _positionHandler = new PositionHandler();

    this.getPositionHandler = function () {
    ...
    this.setPositionError = function (val) {
      precondition(val === null || typeof val === 'string', 'argument is a string');
      _positionError = val;
    };
    ...
    this.setErrorHdlr = function (hdlr) {
      precondition(hdlr === null || isFunction(hdlr), 'hdlr is not a function');
      _errorHdlr = hdlr;
    };

    this.getErrorHdlr = function () {
      return _errorHdlr;
    };
    ...
    this.getDlBwResultHdlr = function () {
      return _dlBwResultHdlr;
    };
    ...
    this.setError = function (err) {
      precondition(err === null || typeof err === 'string', 'err is not a string');
      _error = err;
    };
    ...
    this.getDlBwTestRunning = function () {
      return _dlBwTestRunning;
    };
    ...
    this.setTracer = function (tr) {
      precondition(tr === null || tr instanceof Tracer, 'tr is not an instance of Tracer class');
      _tracer = tr;
    };

    var _dlBwTimer = null;

    this.getDlBwTimer = function () {
      return _dlBwTimer;
    };
    ...
    if (options.onDlBwProgress !== undefined && options.onDlBwProgress !== null) {
      precondition(isFunction(options.onDlBwProgress), 'onDlBwProgress is not a function');
  ...
  start() {
    var getDlBwPrv = async function () {
      ...
      try {
        bw = getDlBw(this.getDlLimitKbytes(), this.getDlBwTestDuration(), this.getUrl(), this.getOnDlBwProgress);
        this.setDlBwTestRunning(true);
        bw.then(async resultBw => {
          this.setDlBw(resultBw);

          if (this.getTracePosition() === true) {
            try {
              const pos = await this.getPositionHandler().getPosition();
              this.getTracer().trace({
                position: pos,
                dlBw: resultBw
              });
              this.setPositionError(this.getPositionHandler().getError());
        ...
        log('err', error.message, 'LppNetworkTracer::start::getDlBwPrv');
        this.setError(error.message);
    ...
    if (this.getTracer() === null) {
      ...
      this.setTracer(new Tracer(this.getSchemaVersion(), this.getDescription(), this.getClientModel(), this.getClientName(), this.getNote(), settings));
    }
...
export { LppNetworkTracer, Position, LppStatus };